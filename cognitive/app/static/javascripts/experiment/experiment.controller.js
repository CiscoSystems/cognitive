(function () {
  ...
  function ExperimentController(
    ...
    WorkflowService, PluginService, ExperimentsService, WhiteboardService, $state, $stateParams) {
    ...
    function addAuthenticationToRequest(requestBody) {
      requestBody['user_id'] = vm.user.id
      requestBody['token'] = vm.user.token
      requestBody['experiment'] = vm.experimentId
      return requestBody
    }

    vm.pluginDropComplete = function (data) {
      var pluginKey = data['pluginKey']
      var resource = PluginService.getPluginResource(pluginKey)
      var definition = PluginService.getPluginList()[pluginKey]
      var request = addAuthenticationToRequest({})

      resource.create(request).$promise
    ...
    vm.linkToExperimentDashboard = function () {
    ...
    vm.show = function () {
      ...
      $http.get('/api/v1/results/?experiment=' + vm.experiment.id + '&component_id=' + focused_node.id)
        .success(function (data) {
          ...
          $modal.open({
            animation: true,
            templateUrl: '/static/javascripts/experiment/result_modal.html',
